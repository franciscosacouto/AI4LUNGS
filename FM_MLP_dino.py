...
class encoder_decoder(L.LightningModule):
    def __init__(self, encoder, survival_head, learning_rate, pos_weight):
        ...
        self.vision_encoder = encoder.backbone
    ...
    def encode_batch(self, images_tensor):
        """
        Takes the image tensor batch (x) and passes it through the DINOv2 classifier wrapper.
        """
        # Call the DINOv2Classifier instance directly with the tensor batch.
        # This will return the feature tensor of shape [B, Num_Features].
        embeddings = self._encoder_wrapper(images_tensor) 
        return embeddings

    # --- FIX 3: Simplify forward pass ---
    def forward(self, x):
        # x is the image tensor batch [B, C, H, W]
        embeddings = self.encode_batch(x) # Call the corrected encode_batch
