...
class encoder_decoder(L.LightningModule):
    ...
    def encode_batch(self, inputs):
        ...
        # 1. Image features
        if getattr(self.config, 'image', False) and 'image' in inputs:
            v_out = self.vision_encoder(inputs['image'])
            img_emb = v_out.pooler_output if hasattr(v_out, "pooler_output") else v_out
            features.append(img_emb.view(img_emb.size(0), -1))

        # 2. Tabular features (Standardized [0,1])
        if getattr(self.config, 'tabular', False) and 'tabular' in inputs:
            features.append(inputs['tabular'])
            
        # 3. Text features
        if getattr(self.config, 'text', False) and 'text' in inputs:
            # If your encoder handles text, add it here
            pass

        if not features:
            raise ValueError("No data modalities enabled in config.")

        return torch.cat(features, dim=1)
    

    def forward(self, inputs):
            # 1. Get the fused feature vector
            fused_features = self.encode_batch(inputs)
            raw_params = self.survival_head(fused_features)
            # 2. Pass to your existing MLP Survival Head
            # Note: Ensure survival_head.input_layer matches fused_features.shape[1]
            log_params = torch.nn.functional.softplus(raw_params) + 1e-4
            return log_params
    ...
    def configure_optimizers(self):
        # Use booleans from config instead of a single 'modality' string
        use_image = getattr(self.config, 'image', False)
        use_tabular = getattr(self.config, 'tabular', False)
        
        param_groups = []

        # 1. ALWAYS include the Survival Head
        for param in self.survival_head.parameters():
            param.requires_grad = True
        param_groups.append({'params': self.survival_head.parameters(), 'lr': self.learning_rate})

        # 2. Vision Encoder Logic (Only if image is enabled)
        if use_image and self.vision_encoder is not None:
            if self.freeze_encoder:
                for param in self.vision_encoder.parameters():
                    param.requires_grad = False
            else:
                for param in self.vision_encoder.parameters():
                    param.requires_grad = True
                # Use lower LR for backbone fine-tuning
                param_groups.append({
                    'params': self.vision_encoder.parameters(), 
                    'lr': self.learning_rate / 10
                })

        # 3. Final Safety Filter: Only pass parameters that actually require gradients
